...
int main(void){
	...
	int flag = 0; /* 1 for the historical command is right, 0 for wrong. */
	int i, j, cnt;
	...
	/* create a piece of shared memory for communication among the processes */
	key_t shmid;
	shmid = shmget(IPC_PRIVATE, sizeof(char) * 10, S_IRUSR|S_IWUSR);
	success = shmat(shmid, 0, 0);
	strcpy(success, "1111111111");
	...
	while(1){
		...
		if (strcmp(args[0], "r") != 0){
			...
			if (args[1] == NULL){ /* only r command */
				...
				else { /* To recall cd command is useless, continue. */
		...
		if (pid < 0){
			...
			if (strcmp(args[0], "r") != 0){
				execvp(args[0], args);
				/* if execvp failed, the process goes below. */
			...
			if (background == 0){
				...
				child_id = wait(NULL);
				while (child_id != pid){
					printf("\033[0;32m%d process is done.\033[0m\n", child_id);
					child_id = wait(NULL);
				}
			}
		}
